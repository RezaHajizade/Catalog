...
namespace Catalog.Apis
{
    public static class CatalogCategoryApi
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                var hasParent = await services.Context.CatalogCategories.AnyAsync(p => p.Id
                == categoryToCreate.ParentId, cancellationToken);
                if (!hasParent)
                {
            ...
            var hasCategory = await services.Context.CatalogCategories.AnyAsync(p => p.Category == categoryToCreate.Category &&
                                                                                p.ParentId == categoryToCreate.ParentId,
                                                                                cancellationToken);
            if (hasCategory)
            {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var category = await Services.Context.CatalogCategories.FirstOrDefaultAsync(p => p.Id == categoryToUpdate.Id, cancellationToken);
        ...
        public static async Task<Results<Ok<CatalogCategoryResponse>, NotFound, BadRequest<string>>> GetCategoryById([AsParameters] CatalogServices services, int id)
        {
            if (id <= 0)
            {
                return TypedResults.BadRequest("Id is not valid.");
        ...
        public static async Task<Results<Ok<IEnumerable<CatalogCategoryResponse>>, BadRequest<string>>> GetCategories([AsParameters] CatalogServices services,
            CancellationToken cancellationToken)
        {
