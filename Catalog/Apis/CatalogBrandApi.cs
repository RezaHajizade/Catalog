...
public static class CatalogBrandApi
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var hasBrand = await services.Context.CatalogBrands.AnyAsync(p => p.Brand == brandToCreate.Brand, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var brand = await services.Context.CatalogBrands.FirstOrDefaultAsync(p => p.Id == brandToUpdate.Id, cancellationToken);
        ...
        [AsParameters] CatalogServices services, int id, CancellationToken cancellationToken)
    {
        ...
        var brand = await services.Context.CatalogBrands.FirstOrDefaultAsync(p => p.Id == id);
    ...
    public static async Task<Results<Ok<IEnumerable<CatalogBrandResponse>>, BadRequest<string>>> GetBrands(
        ...
        CancellationToken cancellationToken)
    {
        var brand = await services.Context.CatalogBrands
                                          .OrderBy(p => p.Id)
                                          .Select(x => new CatalogBrandResponse(x.Id, x.Brand))
                                          .ToListAsync(cancellationToken);

        return TypedResults.Ok<IEnumerable<CatalogBrandResponse>>(brand);
